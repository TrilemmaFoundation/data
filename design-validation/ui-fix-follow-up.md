...
`npm run lint` and the fresh `npm run build` passed with Node 22.18.0.
`npm run test:e2e -- --workers=3`: **87 passed, 11 skipped**. The skipped cases are
10 redundant non-Chromium widths and the opt-in original-reference capture.
Unit tests were not rerun for these presentation changes.

![Guide jump before](images/ui-fix-guide-anchor-before.png)
![Guide jump after](images/ui-fix-guide-anchor-after.png)
![Grouped mobile contribution fields](images/ui-fix-contribute-after.png)

The guide pair uses the same USGS route, reduced-motion setting and 390×844
viewport. The contribution capture also uses 390×844.
