...
## Data changes and evidence

Discovery follows introduction → stationary search/filter controls → results →
Build Path recommendations. Clear-filter feedback is also below the controls, so
it cannot move a focused search field. Local navigation and table headers use the
measured complete shell height. Build Paths use navy feature cards; guides and
metadata remain on light reading surfaces. Guide instructions use 16px prose.

Contribution selectors reflow at 200% text in Firefox and WebKit. Clipboard and
download failures preserve the draft and announce recovery instructions. The
original Foundation wordmark integrity assertion remains alongside new palette
mapping assertions. Metadata, source links, notebook actions and URL state remain.

![Original catalog, mobile](images/catalog-before-390.png)
![Updated catalog, mobile](images/catalog-after-390.png)
![Original catalog, desktop](images/catalog-before-1280.png)
![Updated catalog, desktop](images/catalog-after-1280.png)

Validation: `npm run lint`; `npm test` (533 tests); `npm run build` (includes types);
Playwright route/family/accessibility/performance suite. The final lint/unit/build
run used Node 22.18.0. The existing rapid-filter tests were repeated five times
while investigating one intermittent earlier run; all ten repetitions passed.

Final browser result: **79 passed**, 10 intentionally skipped browser-duplicate cases. See [validation summary](results.json).
